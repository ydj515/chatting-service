...
| room stream shard | `chat:stream:room:{<roomId>}:shard:<shardNo>` | Redis Stream | 필요 | 같은 방의 sharded stream key를 같은 Redis Cluster slot에 배치 |
...
| Redis Streams | room hash tag가 있는 shard별 stream key 유지 | 같은 방의 여러 stream shard를 Redis Cluster `XREADGROUP`로 함께 읽되, 서로 다른 방은 consumer가 room/slot 단위로 나누어 읽음 |
...
> - Redis Cluster node는 `appendfsync everysec`를 사용하므로 node 장애 또는 host crash 시 마지막 fsync 이후 최대 1초의 Redis ingest가 손실될 수 있다. Phase 8.7의 `roomSeq` gap audit가 canonical store 유실 감지 경로를 제공하며, WebSocket heartbeat가 zombie connection 회수 경로를 제공한다.
