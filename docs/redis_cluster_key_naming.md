...
| --- | --- | --- | --- | --- |
| room sequence block | `chat:seq:room:<roomId>` | String counter | 불필요 | 영구 또는 room archive 시 삭제 |
...
| 도메인 | 현재 판단 | 이유 |
| --- | --- | --- |
| Sequence | 단일 key `INCRBY` 유지 | block allocation은 key 1개만 필요하고 gap 허용 정책이 있음 |
