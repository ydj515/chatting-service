...
| --- | --- | --- |
| A | Redis Streams direct lag gauge | `XINFO GROUPS`/`XPENDING` 비용과 polling interval |
| B | Chaos test runbook | Gateway/Worker/Redis/replica 장애 주입과 release blocking 조건 |
| C | WebSocket reconnect chaos orchestration | gateway rolling restart/hard kill을 runner와 연결할 조건 |
| D | Admin search slow query plan capture | cold p99 실패 시 `EXPLAIN (ANALYZE, BUFFERS)` 수집 조건 |
