...
| --- | --- | --- |
| A | Admin search warm/cold latency gate | warm p95와 cold p99 측정 조건 분리 |
| B | Redis Streams direct lag gauge | `XINFO GROUPS`/`XPENDING` 비용과 polling interval |
| C | Chaos test runbook | Gateway/Worker/Redis/replica 장애 주입과 release blocking 조건 |
| D | WebSocket reconnect chaos orchestration | gateway rolling restart/hard kill을 runner와 연결할 조건 |
...
- 다음 슬라이스는 Admin search warm/cold latency gate로 둘 것인가?
- Redis Streams direct lag gauge를 application poller로 둘지, Redis exporter/Prometheus query로 둘지 결정할 것인가?
