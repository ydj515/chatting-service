...
| `PATCH` | `/api/admin/rooms/{roomId}/policy` | bounded live feed, room/user rate limit, slow mode, moderator priority 정책 override. 기본적으로 `autoPolicyEnabled=false`가 되어 room-policy worker가 해당 방의 수동 정책을 덮어쓰지 않음 |
