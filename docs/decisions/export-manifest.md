...
상태: 미구현 제안. Chunk pagination과 cursor checkpoint는 구현되어 있지만 crash 시 중복 가능성이 남는다. 운영 사용 전에 중복 허용 여부를 결정하고, 정확한 결과가 필요한 경우 atomic manifest를 적용·검증해야 한다.
...
5. 운영자가 job을 requeue하고 기존 staging 파일을 사용할 수 있으면 checkpoint cursor부터 재개한다. RUNNING job 자동 lease 회수는 제공하지 않는다.
