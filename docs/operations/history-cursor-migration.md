...
Public room history uses a roomSeq boundary. The opaque token carries three fields, but does not change the public query into admin-style tuple pagination. Admin room history orders by `(roomSeq, createdAt, messageId)` descending; admin search orders by `(createdAt, roomSeq, messageId)` descending.
