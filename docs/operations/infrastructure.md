...
Redis Cluster host port는 `127.0.0.1`에만 bind한다. `infra/redis/redis-cluster.conf`는 `appendonly yes`, `appendfsync everysec`, `cluster-preferred-endpoint-type hostname`을 사용한다. 마지막 fsync 이후의 데이터 손실 가능성을 운영 복구 계획에 반영한다.

`CHAT_REDIS_STREAMS_MAX_LEN=1000000`은 stream별 수락 용량 제한이다. 모든 기존 consumer group이 ACK한 prefix만 회수하며 필수 writer/fanout group이 없거나 공간이 부족하면 신규 수락을 거부한다. 미처리 entry를 MAXLEN으로 제거하지 않는다. `0` 이하이면 제한을 끄며 `CHAT_REDIS_STREAMS_MAX_LEN_APPROXIMATE`는 현재 사용하지 않는 호환 설정이다. [메시지 계약](../architecture/messaging.md)과 [전환 절차](migrations.md)를 따른다. Lag/pending, Redis memory, gap audit와 heartbeat를 함께 관측한다.

Prometheus alert는 Compose `cluster` profile의 Alertmanager로 전달된다. `severity="warning"`은 Slack-compatible webhook receiver로, `severity="critical"` 또는 `release_blocking="true"`는 PagerDuty receiver로 라우팅한다. Prometheus는 Alertmanager 자체 metric도 scrape하며, PagerDuty notification 실패는 `AlertmanagerPagerDutyNotificationFailures` warning alert로 감지해 Slack에 보낸다. 실제 secret 파일 주입, silence, synthetic delivery smoke 절차는 [on call](on-call.md)를 따른다. `alert-smoke` profile은 별도 `prometheus-alert-smoke` 서비스를 띄워 기본 Prometheus와 분리된 synthetic alert rule만 추가 평가한다.
...
전체 시스템 아키텍처 구조와 구체적인 메시지 전송 및 실시간 전파 흐름은 [overview](../architecture/overview.md)를 참고하시기 바랍니다.
...
상세 원인, 대안, 완료 기준은 [backlog](backlog.md)의 Nginx 재생성 대응 항목을 따릅니다.
...
| 문서 | 설명 |
| --- | --- |
| [redis keys](../architecture/redis-keys.md) | Redis Cluster key naming, hash slot, hash tag 정책 |
| [observability](observability.md) | Phase 7 dashboard metric, alert, cardinality 정책 |
| [reconnect load](../testing/reconnect-load.md) | 정상 reconnect 실패율과 ticket rate limit 원자성 판단용 synthetic load test 시나리오 |
| [admin search latency](../testing/admin-search-latency.md) | 관리자 검색 warm p95와 cold p99 release gate 측정 절차 |
| [admin search plans](../testing/admin-search-plans.md) | 관리자 검색 cold p99 실패 시 PostgreSQL 실행 계획 수집 절차 |
| [streams lag metrics](streams-lag-metrics.md) | Redis Streams group lag/pending direct gauge 운영 기준 |
| [streams lag alerts](streams-lag-alerts.md) | Redis Streams lag/pending Prometheus alert rule 기준 |
| [on call](on-call.md) | Alertmanager Slack/PagerDuty severity routing과 secret 주입 절차 |
| [overview](../architecture/overview.md) | 분산 웹소켓 서버 아키텍처 및 실시간 채팅 데이터 흐름 (Mermaid 다이어그램) |
