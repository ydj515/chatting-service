...
이 문서는 Phase 7에서 gateway 장애(rolling restart / hard kill)를 reconnect storm과 시간적으로 겹쳐 주입하고, 장애 구간 reconnect gate를 release-blocking으로 판정하는 orchestrator의 실행 절차와 기준을 정의한다.
...
release gate는 장애 구간 storm의 reconnect gate를 그대로 쓴다.
...
- `releaseBlocking = !reconnect.ok` — 장애 구간 storm gate가 하나라도 깨지면 release-blocking.
...
dry-run은 `reconnect: null`, `injectionOffsetMs: null`이며 `plan`(fault step + reconnect 인자)을 추가로 출력한다.
...
4. storm summary의 `ok`/`failedGates`로 release 여부를 기록한다.
...
- 주입→storm 완료 wall-clock에 대한 복구 SLO gate를 추가할 것인가?
