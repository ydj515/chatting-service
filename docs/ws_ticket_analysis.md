...
세부 완료 기준은 [production_hardening_tasks.md](./production_hardening_tasks.md)의 `WebSocket Ticket Rate Limit Lua Script 전환` 항목을 따른다.
...
- Redis Cluster에서 여러 key를 한 Lua script로 묶는 기능이 필요해지는 시점에 hash tag 정책을 별도 문서로 분리할까?
