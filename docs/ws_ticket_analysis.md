...
2026-06-13 기준 Phase 2.5에서 one-time ticket 도입은 완료했다. 이후 Production Hardening으로 ticket 발급 rate limit의 Redis `INCR + EXPIRE` 경로를 Lua script 기반 원자 처리로 승격했다. 세부 task는 [production_hardening_tasks.md](./production_hardening_tasks.md)를 따른다.
...
| WebSocket 인증               | docker/production에서는 one-time ticket 검증 후 `userId`를 session attribute에 저장         |
| Browser WebSocket token 전달 | Browser WebSocket API 제약 때문에 짧은 TTL의 one-time ticket을 URL query parameter로 사용    |
| 토큰 종류                      | 자체 HMAC-SHA256 서명 토큰, 형식: `v1.<base64url-payload>.<base64url-signature>`       |
| 기본 토큰 TTL                  | 12시간                                                                           |
| 토큰 재사용                     | REST API는 session token, WebSocket은 단일 사용 ticket으로 분리                         |
| 로그 보호                      | Nginx access log는 `$request_method $uri $server_protocol`만 기록해 query string 제외 |
| 토큰 폐기                      | 미지원, stateless 검증                                                              |
| One-time ticket            | Phase 2.5에서 구현 완료                                                               |
| 재연결                        | 클라이언트 exponential backoff, 재연결 직전에 새 ticket 발급                                |

현재 Phase 1에서 가장 직접적인 유출 경로였던 Nginx access log의 query string 기록은 완화되어 있다. Phase 2.5 이후 docker/production WebSocket URL에는 reusable session token 대신 one-time ticket만 포함된다.

---

## 3. 현재 위험 요소

Phase 2.5 이전 구조 또는 local/dev 호환 fallback을 켠 구조에서 `sessionToken`이 URL에 노출될 수 있는 경로는 다음과 같다.
...
> [!WARNING]
> Phase 2.5 이전 WebSocket URL에 들어가던 값은 12시간 TTL의 재사용 가능한 session token이었다. local/dev fallback을 운영에 열어두면 같은 위험이 되살아나므로 production에서는 one-time ticket만 허용해야 한다.
...
| 속성                 | 기존 Session Token 방식    | One-Time Ticket 방식                 |
|--------------------|------------------------|------------------------------------|
...
### 범위
- [x] `POST /api/ws-tickets` 엔드포인트 추가
- [x] session token 인증 후 WebSocket 전용 ticket 발급
- [x] ticket 원문이 아닌 `sha256(ticket)` 기준으로 Redis 저장
- [x] Redis key TTL 30초 적용
- [x] ticket consume은 Redis `GETDEL`에 해당하는 `getAndDelete`로 원자 처리
- [x] ticket value에는 `userId`, 발급 시각, 만료 시각만 저장
- [x] client IP는 ticket payload가 아니라 ticket 발급 rate limit key 계산에만 사용
- [x] WebSocket handshake는 docker/production 설정에서 `ticket`만 허용
- [x] 기존 `token` query parameter는 local/dev 호환 flag로만 허용
- [x] 클라이언트는 WebSocket connect/reconnect 직전에 ticket을 발급받도록 수정
- [x] 재연결 시 기존 ticket 재사용 금지
- [x] ticket 발급 endpoint rate limit 추가
- [x] ticket 발급/소비/만료/재사용 실패 metrics 추가
- [x] OpenAPI, configuration, verify script 갱신
- [x] 통합 테스트와 보안 회귀 테스트 추가
...
chat:ws-ticket:{sha256(ticket)} -> {
  "userId": 123,
  "issuedAtEpochSecond": 1781266800,
  "expiresAtEpochSecond": 1781266830
}
TTL: 30s
```

ticket 원문은 Redis에 저장하지 않는다. 로그나 Redis dump가 유출되더라도 즉시 ticket으로 사용할 수 없게 하기 위함이다. `clientIp`는 발급 API의 rate limit key 계산에만 사용하고 ticket payload에는 저장하지 않는다. 모바일 네트워크, NAT, proxy, 로드밸런서 경유 환경에서는 handshake 시점 IP가 발급 시점 IP와 달라질 수 있으므로 IP 바인딩을 기본 검증 조건으로 두지 않는다.
...
Redis `INCR`와 `EXPIRE`를 분리해 rate limit counter를 구현하면, 두 명령 사이 장애로 TTL 없는 key가 남을 수 있다. 현재 구현은 Lua script로 `INCR + PEXPIRE + TTL repair`를 Redis 내부에서 원자 처리한다.

Redis Cluster hash slot 관점에서는 user 기준 rate limit과 IP 기준 rate limit을 각각 **단일 key Lua script**로 실행한다. script 호출 1회에 `KEYS[1]`만 전달하고, Lua 내부에서 다른 key를 조합하지 않으므로 현재 구조는 cross-slot 문제를 만들지 않는다. user key와 IP key를 하나의 script에 함께 넣는 방식은 모든 key를 같은 hash slot에 배치해야 하므로 현 단계의 기본안으로 두지 않는다.

user limit과 IP limit의 완전 원자성이 필요한지는 Phase 7에서 운영 지표로 판단한다. 현재 단일 key script 방식은 fail-closed라 제한 우회 위험은 낮지만, IP limit 거부 과정에서 user counter가 보수적으로 소모되어 정상 reconnect UX에 영향을 줄 수 있다. 기본 운영 기준은 현재 방식을 유지하는 것이다. 전체 정상 reconnect ticket 발급 성공률이 rolling 15분 기준 `99.9%` 이상이고, rate limit으로 인한 정상 reconnect 실패율이 `0.1%` 이하이며, NAT/proxy/mobile carrier cohort p95가 `0.3%` 이하이면 완전 원자성을 도입하지 않는다. 세부 완료 기준은 [production_hardening_tasks.md](./production_hardening_tasks.md)의 `WebSocket Ticket Rate Limit Lua Script 전환` 항목을 따르고, Redis key naming 기준은 [redis_cluster_key_naming.md](./redis_cluster_key_naming.md)를 따른다.
---
...
- ticket rate limit은 Lua script로 `INCR + PEXPIRE + TTL repair`를 원자 처리한다.
- ticket 원문이 Redis key/value에 저장되지 않는다.
- ticket payload에는 client IP를 저장하지 않는다.
...
- `chat.websocket.ticket.issue.latency` timer가 outcome별로 dashboard에 표시됨
- `chat.websocket.ticket.rate_limit.script.failures` counter가 user/IP scope별로 dashboard와 alert에 연결됨
- Redis latency 증가 시 WebSocket handshake p95/p99가 관측됨
- Redis 장애 시 ticket 발급/handshake가 fail-closed로 실패하고 알림이 발생함
- Phase 7 dashboard metric 목록은 [observability_metrics.md](./observability_metrics.md)를 따른다.
...
- production/docker 설정은 기존 `token` query fallback을 비활성화한다. local/dev는 `CHAT_AUTH_WEB_SOCKET_TICKET_SESSION_FALLBACK_ENABLED=true`로 전환 기간 호환을 열 수 있다.
- ticket TTL 기본값은 30초로 둔다.
- ticket 발급 rate limit 저장소는 Redis로 둔다. 기본값은 사용자 10회/분, IP 60회/분이다.
- `POST /api/ws-tickets`는 `chat-api-application`에 둔다.
- E2E 검증 스크립트는 WebSocket URL의 `ticket`/`token` query 값을 error message에서 redaction한다.
- ticket 발급 rate limit은 Lua script 기반 원자 처리로 동작한다.
- Redis Cluster에서는 ticket rate limit Lua script를 단일 key 단위로 실행하므로 현재 구조에서는 hash tag가 필수는 아니다.
- user/IP limit의 완전 원자성 필요 여부는 Phase 7 운영 지표로 판단하되, 기본 기준은 현재 단일 key Lua script + fail-closed 유지다.

---

## 13. 후속 질문 (Next Questions)

- ticket TTL은 기본 30초로 유지하되, 운영 지표에 따라 줄이는 방향으로 갈까?
- Phase 7에서 정상 reconnect 실패율이 기준을 넘는 경우 limit/window/burst 튜닝을 먼저 적용할까, cohort별 정책 분리를 먼저 적용할까?
