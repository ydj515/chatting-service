# WebSocket Reconnect Chaos Orchestration
...
기존 자산을 연결한다. `scripts/phase7-reconnect-load.mjs`(synthetic reconnect storm)를 child로 돌리면서 gateway에 장애를 주입한다. 단일 컨테이너 복구만 보는 [chaos runbook](chaos.md)과 달리, 이 슬라이스는 장애 **구간에 storm gate가 유지되는지**를 본다. reconnect gate 기준은 [reconnect load test 시나리오](reconnect-load.md) 4.2 / 4.3을 그대로 따른다.
...
storm과 장애가 겹치지 않으면 chaos 의미가 사라진다. orchestrator는 [takeover smoke](fanout-takeover.md)의 metadata-file 패턴을 따른다.
