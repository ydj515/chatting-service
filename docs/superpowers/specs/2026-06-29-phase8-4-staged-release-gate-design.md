...
- release gate를 `1k`, `3k`, `5k`, `7k`, `10k` 단계로 실행해 어느 규모까지 안정적으로 통과하는지 확인한다.
- 기본 단계는 `1k -> 3k -> 5k -> 7k -> 10k` 순서로 실행한다.
...
- 기본 동작은 fail-fast로 한다. 예를 들어 `5k`에서 실패하면 `7k`와 `10k`는 실행하지 않는다.
- 필요하면 `--stages 1000,2000,4000,8000,10000`처럼 단계 목록을 커스터마이즈할 수 있게 한다.
...
- `1k` stage 실행
- `1k`가 통과하면 `3k` stage 실행
- `3k`가 통과하면 `5k` stage 실행
- `5k`가 통과하면 `7k` stage 실행
- `7k`가 통과하면 `10k` stage 실행
- 실패 시 이후 stage는 생략
...
- `1k`는 기능/구성 회귀를 빨리 잡고, `3k`는 초기 확장 구간을 확인하며, `5k`는 기존 5,104 ticket 진행 지점 주변을 다시 검증한다.
- `7k`는 5k 이후에도 선형적으로 버티는지 확인해 10k 실패 시 병목 구간을 더 좁히는 중간 지점이다.
...
stage별 snapshot을 별도로 남긴다. 3k가 통과하고 5k가 실패하면 3k snapshot은 성공 artifact로 보존되고, 5k는 실패 원인을 남긴다.
...
- 기본 `1k/3k/5k/7k/10k` 총 viewer 준비 작업은 `O(26000)`이다.
- 기본 `1k/3k/5k/7k/10k` 총 메시지 전송 목표는 `O(1560000)`이다.
...
> - 특정 stage 실패가 곧 서비스 한계라는 뜻은 아니다. nginx, app, DB, Redis, load generator 중 어느 계층인지 artifact로 분리해야 한다.
...
| 대안 | 장점 | 단점 | 판단 |
| --- | --- | --- | --- |
| `1k/3k/5k/7k/10k` 세밀 단계 | 병목 구간을 더 촘촘히 찾을 수 있고, 5k 이후 10k 전까지의 기울기를 볼 수 있다 | `1k/5k/10k`보다 실행 시간이 길고 환경 피로도가 커진다 | 선택 |
| `1k/5k/10k` 고정 단계 | 실행 시간이 짧고 보고서에 설명하기 쉽다 | 5k와 10k 사이의 정확한 한계는 추가 실행이 필요하다 | 제외 |
...
- 7k는 통과하고 10k에서 실패하면 추가 탐색 stage를 `8000` 또는 `9000` 중 어디부터 넣을 것인가?
