...
node scripts/load-chat.mjs --room hot --viewers 3 --messages-per-sec 100 --duration 30 --assert-room-seq-order
...
- Phase 6 fanout owner 방향 확정: production fanout worker scale-out은 Redis TTL lease 기반 방별 owner 모델로 진행한다. Redis Streams consumer group만으로 fanout worker를 늘리면 같은 방 record가 여러 worker에서 동시에 batch publish되어 live feed 순서가 뒤집힐 수 있다. 따라서 owner worker 하나만 같은 room/stream shard의 `XREADGROUP`, publish, `XACK`를 수행하고, worker 장애 시 TTL 만료 후 다른 worker가 takeover한다.
