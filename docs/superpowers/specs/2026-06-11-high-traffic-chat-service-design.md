...
```text
presence:user:{userId} -> set of session ids, TTL
presence:room:{roomId}:gateways -> set of gateway ids, TTL
gateway:{gatewayId}:rooms -> set of room ids, TTL
rate:user:{userId}:sec:{epochSecond} -> count, TTL
rate:room:{roomId}:sec:{epochSecond} -> count, TTL
seq:room:{roomId} -> last allocated room sequence
stream:room:{roomId}:shard:{shardNo} -> Redis Stream
```
...
- Redis Streams producer를 추가한다.
- `stream:room:{roomId}:shard:{streamShard}` key 전략을 구현한다.
- `streamShard = hash(roomId 또는 messageId) % streamShardCount` 정책을 명시한다.
