...
- fan-out tier를 방별 owner lease로 순서를 유지한 채 수평 확장 가능하게 설계한다.
...
현재 코드는 역할별 실행 모듈을 이미 가진 멀티모듈 구조다. Redis Streams ingest, Message Writer Worker, Hot Room Fanout Worker, partitioned message store, admin search까지 단계적으로 구현되었고, 기존 `chat-application`은 실서비스 배포 대상이 아니라 통합 테스트나 로컬 fallback 용도로만 남긴다.

현재 실시간 흐름은 다음에 가깝다.

1. 클라이언트가 WebSocket Gateway 인스턴스에 연결한다.
2. 메시지 전송 요청이 들어오면 서버는 방별 Redis counter에 `INCR 1`을 수행해 `roomSeq`를 발급하고 Redis Streams에 append한다.
3. Streams append가 성공하면 클라이언트에는 `MESSAGE_ACCEPTED`를 반환한다.
4. `MessageWriterWorker`가 Redis Streams를 읽어 PostgreSQL canonical store에 저장한다.
5. `HotRoomFanoutWorker`가 Redis Streams를 읽어 batch를 만들고 Redis Pub/Sub room channel로 발행한다.
6. 각 WebSocket Gateway는 구독 중인 room channel 이벤트를 받아 로컬 세션으로 전달한다.

현재 구조의 장점은 단순하고 이해하기 쉽다는 점이다. 다만 실서비스 고트래픽에서는 다음 지점이 병목이 된다.

- Redis Streams와 PostgreSQL 저장 경로는 분리되었지만, fanout worker를 여러 대로 늘릴 때 같은 방 메시지가 여러 worker에서 동시에 발행되면 방별 표시 순서가 깨질 수 있다.
- Redis Pub/Sub은 내구성이 없으므로 fanout worker가 publish 후 ack 직전에 죽는 경우 중복 전파가 가능하다. 클라이언트는 `messageId` deduplication과 `roomSeq` 정렬을 유지해야 한다.
- 로컬 fan-out은 `roomId -> sessions` 인덱스를 사용해야 하며, 전체 로컬 사용자 세션을 순회하면 안 된다.
...
flowchart LR
    ...
    HotFanout <--> FanoutOwner["Redis TTL Lease\nroom fanout owner"]
...
Production fanout ownership 정책:

- `MessageWriterWorker`는 Redis Streams consumer group 분배만으로 수평 확장해도 된다. 저장 경로는 `messageId` 멱등성으로 중복 처리를 흡수할 수 있기 때문이다.
- `HotRoomFanoutWorker`는 consumer group 분배만으로 확장하지 않는다. 같은 방의 record가 여러 worker에서 경쟁 처리되면 publish 순서가 뒤집힐 수 있고, batch 단위로는 클라이언트가 그 순서를 복구하기 어렵다.
- Phase 6의 첫 번째 production gate는 Redis TTL lease 기반 방별 fanout owner다. owner key는 `chat:fanout:owner:room:<roomId>:shard:<streamShard>`를 사용한다.
- owner value는 `<workerId>:<leaseToken>` 형식으로 둔다. `leaseToken`은 worker가 owner를 획득할 때마다 새로 만드는 fencing token이다.
- owner 획득은 `SET key value NX PX <ttlMillis>`로 수행한다. owner 갱신과 해제는 현재 value가 자신의 token과 일치할 때만 `PEXPIRE` 또는 `DEL`하는 Lua script로 처리한다.
- 기본값은 TTL `10s`, renew interval `3s`다. GC pause, network stall, Redis latency를 보고 운영에서 조정한다.
- fanout worker는 방/stream shard의 owner일 때만 `XREADGROUP`, batch publish, `XACK`를 수행한다.
- publish 직전과 ack 직전에 owner token을 다시 확인한다. token mismatch가 나면 stale owner로 보고 publish/ack를 중단한다.
- worker가 죽으면 TTL 만료 후 다른 worker가 owner를 획득하고 pending entry를 claim해 이어서 처리한다.
- HPA로 fanout worker pod를 늘리는 목적은 같은 방 record를 쪼개 처리하는 것이 아니라, 여러 방의 owner를 여러 worker에 분산하는 것이다.
...
- Fanout owner lease는 방별 publish 순서를 지키기 위한 장치이지, exactly-once delivery를 보장하는 장치가 아니다. 중복 가능성은 `messageId` deduplication으로 흡수한다.
...
- Gateway별 구독 방 목록 저장
- room routing 정보 저장
- message admission rate limit / slow mode counter 저장
- accepted room traffic counter 저장
- room sequence counter 발급
- Redis Streams ingest buffer
- Redis Sharded Pub/Sub fan-out
- room fanout owner TTL lease
...
chat:admission:room:{<roomId>}:rate:room:<epochSecond> -> room rate count, TTL
chat:admission:room:{<roomId>}:rate:user:<userId>:<epochSecond> -> user rate count, TTL
chat:admission:room:{<roomId>}:slow:user:<userId> -> slow mode guard, TTL
chat:room-traffic:{<roomId>}:sec:<epochSecond> -> accepted traffic count, TTL
chat:room-traffic:active-rooms -> active traffic room index
chat:seq:room:<roomId> -> last allocated room sequence
chat:stream:room:<roomId>:shard:<shardNo> -> Redis Stream
chat:fanout:owner:room:<roomId>:shard:<shardNo> -> fanout owner lease, TTL
```

기본 원칙은 단일 key 명령과 단일 key Lua script에는 hash tag를 강제하지 않는 것이다. 다만 message admission은 room rate limit, user rate limit, slow mode를 한 메시지 단위로 원자 판단해야 하므로 `{<roomId>}` hash tag로 같은 Redis Cluster slot에 묶는다. room traffic counter도 room별 60초 window를 읽기 때문에 `{<roomId>}` hash tag를 사용한다.
...
4. Redis admission policy가 `room_storage_configs`의 room/user rate limit과 slow mode를 확인한다.
5. Room Sequence Service가 `messageId`, `roomSeq`를 발급한다.
6. 메시지를 Redis Streams에 append한다.
7. append 성공 후 accepted room traffic counter를 best-effort로 기록하고, 서버는 송신자에게 `MESSAGE_ACCEPTED` ACK를 반환한다.
...
- 같은 방/stream shard의 fanout publish는 Redis TTL lease 기반 owner worker 하나만 수행한다.
...
fanout worker를 HPA로 늘릴 때는 owner lease가 먼저 필요하다. Redis Streams consumer group은 record 중복 처리를 줄이지만, 같은 방의 record를 여러 worker가 동시에 읽고 publish하는 순간 batch 간 순서가 뒤집힐 수 있다. 따라서 fanout scale-out의 단위는 "같은 방 안의 메시지 병렬 처리"가 아니라 "방별 owner를 여러 worker에 분산"하는 것이다.
...
- hot room slow mode
- 동일 메시지 반복 제한
- 운영자/스트리머/moderator 우선권. Phase 6 구현은 `moderatorPriority=true`일 때 `MemberRole.OWNER/ADMIN`이 Redis admission rate limit과 slow mode를 우회하고, false일 때는 일반 멤버처럼 제한 대상이 된다.
...
```text
WORKER_ROLES=message-writer,fanout,admin-export,room-policy
```

운영 중 특정 role만 병목이 되면 같은 실행 모듈을 role별 Deployment로 나눠 띄운다.

`fanout` role을 2개 이상 띄우는 production 배포에서는 Phase 6의 Redis TTL owner lease를 활성화해야 한다. owner lease 없이 fanout worker replica만 늘리면 같은 방 메시지가 서로 다른 worker에서 batch publish되어 live feed 순서가 뒤집힐 수 있다. `message-writer` role은 DB 멱등 저장으로 중복 처리를 흡수할 수 있으므로 consumer group 기반 scale-out을 우선 적용한다.
...
- fanout batch size
- fanout worker lag
- fanout owner lease acquire/renew/lost count
- fanout owner rooms gauge
- fanout owner takeover count
- message admission rejected count by reason
...
Phase 6   Redis TTL fanout owner lease, hot room 운영 정책, downgrade/rate limit/live feed
...
- `WORKER_ROLES=message-writer,fanout,admin-export,room-policy`으로 worker role을 선택 실행할 수 있게 한다.
...
- Fanout Worker replica를 여러 대로 늘리는 production 배포는 Phase 6의 방별 owner lease 적용 후 진행한다.
...
### Phase 6. Redis TTL fanout owner lease와 Hot room 운영 정책
...
- Phase 3 fanout worker 경로가 안정화되어 있고, worker를 여러 replica로 늘릴 계획이 있다.

목표:

- Redis TTL lease 기반 방별 fanout owner를 도입해 fanout worker를 여러 대로 늘려도 같은 방 live feed 순서가 깨지지 않게 한다.
...
- `FanoutOwnerLeaseService`를 추가한다.
- owner lease key는 `chat:fanout:owner:room:<roomId>:shard:<streamShard>`를 사용한다.
- lease value는 `<workerId>:<leaseToken>`으로 저장한다. `leaseToken`은 owner 획득마다 새로 발급해 fencing token으로 사용한다.
- owner 획득은 `SET NX PX`로 처리하고, renew/release는 현재 value가 자신의 token과 일치할 때만 동작하는 Lua script로 처리한다.
- 기본 TTL은 `10000ms`, renew interval은 `3000ms`로 둔다.
- `renewIntervalMillis=3000`은 실제 TTL renew cadence다. 초기 구현에서는 설정값은 있었지만 owner 보유 중 매 poll마다 renew를 호출해 운영 기준이 적용되지 않았으므로, lease별 마지막 renew 시각을 기준으로 3초가 지난 경우에만 renew한다.
- `HotRoomFanoutWorker`는 room/stream shard owner인 경우에만 `XREADGROUP`, pending claim, batch publish, `XACK`를 수행한다.
- publish 직전과 ack 직전에 owner token을 다시 검증한다. token mismatch는 `lease_lost`로 계측하고 publish/ack를 중단한다.
- owner가 죽거나 GC pause로 lease를 잃으면 TTL 만료 후 다른 worker가 owner를 획득하고 pending entry를 claim해 재처리한다.
- fanout worker HPA는 Redis Streams lag뿐 아니라 owner room count, lease lost count, takeover count를 함께 본다.
...
- slow mode와 moderator priority를 추가한다. subscriber-only mode는 Phase 6 스키마에 `subscriber_only` 정책 컬럼으로 준비하지만, 현재 `MemberRole`에 subscriber 역할/구독 도메인이 없어 메시지 수락 강제 로직은 넣지 않는다.
- 방별 `roomRateLimitPerSecond`, 사용자별 `userRateLimitPerSecond`, 신규/비인증 사용자 제한을 함께 적용한다.
- `MemberRole.OWNER`와 `MemberRole.ADMIN`은 `moderatorPriority=true`일 때 Redis admission rate limit과 slow mode를 우회한다. `moderatorPriority=false`인 방에서는 우회하지 않는다.
- `room-policy` worker는 Redis accepted traffic counter의 최근 60초 window에서 현재 초당 메시지 수와 p95를 계산해 `room_storage_configs`에 heat/live feed/rate/slow-mode 정책을 upsert한다.
- `room-policy` worker는 `RoomPolicySignalProvider`가 제공하는 writer lag, fanout lag, Gateway send queue depth를 traffic snapshot에 합성할 수 있다. 현재 기본 provider는 0을 반환하므로, production에서는 Redis Streams lag와 Gateway queue depth 계측 provider를 연결해야 `OVERLOAD` 판정이 실제 시스템 lag를 반영한다.
- bounded live feed 정책을 클라이언트에 구현한다.
- 클라이언트는 기본적으로 최근 `1,000개`와 최근 `60초` window를 동시에 적용해 렌더링한다.
- 현재 server-side backpressure 구현은 bounded outbound queue overflow 시 해당 WebSocket을 `1013 Outbound queue full`로 닫아 재연결과 history/gap fill 경로를 사용하게 한다. 별도 gap 안내 event는 연결을 유지하는 drop-oldest 전략으로 바꿀 때 추가한다.

이번 Phase에서 하지 않을 것:

- PostgreSQL 저장률을 낮추는 정책은 만들지 않는다.
- hot room이라고 해서 수락된 메시지를 canonical store에서 누락시키지 않는다.
- 같은 방의 ordered fanout을 여러 owner가 병렬 publish하는 구조는 만들지 않는다. 방 내부 fanout 병렬화가 필요해지는 시점에는 per-lane ordering과 클라이언트 merge 계약을 별도 설계한다.

주요 파일:

- `chat-domain/src/main/kotlin/dto/RoomPolicyDto.kt`
- `chat-persistence/src/main/kotlin/service/FanoutOwnerLeaseService.kt`
...
- 같은 room/stream shard에는 동시에 active fanout owner가 1개만 존재한다.
- fanout worker replica를 2개 이상 띄워도 같은 방의 batch publish 순서가 `roomSeq` 순서를 따른다.
- owner worker kill, GC pause simulation, Redis reconnect 후에도 TTL 만료와 takeover로 fanout이 재개된다.
...
```bash
./gradlew test --no-daemon
docker compose up -d --scale chat-worker-app-1=2
node --test scripts/lib/loadChatPlan.test.mjs
node --check scripts/load-chat.mjs
node scripts/load-chat.mjs --room hot --viewers 3 --messages-per-sec 100 --duration 30 --min-received-ratio 0.9 --assert-room-seq-order
node --check scripts/phase6-fanout-takeover-smoke.mjs
node scripts/phase6-fanout-takeover-smoke.mjs --room phase6 --viewers 3 --messages-per-sec 20 --duration 30 --kill-after 5 --drain-wait 12
# Phase 6에서 생성한 뒤 실행한다.
node scripts/load-chat.mjs --room hot --viewers 10000 --messages-per-sec 10000 --duration 60 --min-received-ratio 0.9 --assert-room-seq-order
```
...
- `chat-persistence/src/main/kotlin/redis/RedisMessageStreamProducer.kt`, 신규 `RoomSeqGapAuditRepository`, `RoomSeqGapAuditWorker`, `RoomSeqGapAuditMetrics`
...
# 생존성
./gradlew :chat-persistence:test --tests com.chat.persistence.redis.RedisMessageStreamProducerTest --tests com.chat.persistence.repository.RoomSeqGapAuditRepositoryTest --tests com.chat.persistence.service.RoomSeqGapAuditMetricsTest --tests com.chat.persistence.service.RoomSeqGapAuditWorkerTest --no-daemon
...
- Phase 6 fanout owner 구현 완료: production fanout worker scale-out은 Redis TTL lease 기반 방별 owner 모델로 진행한다. Redis Streams consumer group만으로 fanout worker를 늘리면 같은 방 record가 여러 worker에서 동시에 batch publish되어 live feed 순서가 뒤집힐 수 있다. 따라서 owner worker 하나만 같은 room/stream shard의 `XREADGROUP`, publish, `XACK`를 수행하고, worker 장애 시 TTL 만료 후 다른 worker가 takeover한다. 운영 기본값은 TTL 10초, renew interval 3초로 확정했으며, owner kill takeover smoke에서 `sent=600`, `receivedPerViewer=[600,600,600]`, `assertedRoomSeqOrder=true`를 확인했다.
- Phase 6 message admission 구현 완료: hot room 수락량 제어는 `room_storage_configs`의 `room_rate_limit_per_second`, `user_rate_limit_per_second`, `slow_mode_seconds`, `moderator_priority`를 source of truth로 삼는다. 메시지 hot path에서는 room admission policy를 캐시하고, Redis Cluster hash tag가 있는 `chat:admission:room:{<roomId>}:...` key 3개를 Lua script 하나로 원자 판단한다. 제한 초과는 sequence 발급과 Streams append 전에 `MessageAdmissionRejectedException`으로 fail-closed 처리하며 REST API는 HTTP 429, WebSocket은 `MESSAGE_ADMISSION_REJECTED` 에러로 응답한다. `MemberRole.OWNER/ADMIN`은 `moderatorPriority=true`일 때만 admission 제한을 우회한다.
- Phase 6 자동 downgrade 구현 완료: Streams append에 성공한 accepted message만 `chat:room-traffic:{<roomId>}:sec:<epochSecond>` counter에 best-effort로 기록한다. `room-policy` worker는 active room index를 읽고 최근 60초 snapshot과 `RoomPolicySignalProvider`의 lag 신호를 `RoomHeatClassifier`에 전달해 `NORMAL`, `HOT`, `VERY_HOT`, `OVERLOAD` 정책을 `room_storage_configs`에 자동 upsert한다. 기본 downgrade 값은 `VERY_HOT=500개/30초`, `OVERLOAD=300개/15초`다. 단, admin policy override가 적용되어 `auto_policy_enabled=false`인 방은 자동 upsert가 기존 수동 정책을 덮어쓰지 않는다.
