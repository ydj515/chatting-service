...
DB 제약 `ck_user_sanctions_type`은 `MUTE`, `BAN`, `SUSPEND`를 허용한다.
...
순서는 sanction 저장이 먼저다. revocation이나 force logout publish가 실패하면 트랜잭션 전체를 실패시키는 정책을 사용한다. suspend가 DB에 저장됐는데 token은 살아 있는 상태가 되는 것을 피하기 위해서다.
...
2. `SessionTokenService.issueToken(userId)`가 `issuedAt`, `expiresAt`, `jti`를 포함한 HMAC token을 발급한다.
3. client는 bearer token으로 API와 WebSocket ticket API를 호출한다.
...
4. audit log를 기록한다.
5. `SessionTokenService.revokeUserTokens(userId)`가 user revoke-after marker를 Redis에 저장한다.
6. `SessionControlPublisher`가 Redis control topic으로 force logout event를 publish한다.
...
- Redis revoke 기록 실패: logout/suspend 요청은 실패로 처리한다.
- Redis force logout publish 실패: suspend 요청은 실패로 처리한다.
