...
| roomSeq client-visible order | 위반 없음 |
| stream shard 분산 | hot room 기준 최소 `16`개 stream shard 관측 |
| fanout p95 | `500ms` 이하 |
| Redis Streams group lag | release gate 실행 종료 후 지속 증가 없음 |

부하 발생은 기존 `scripts/load-chat.mjs`를 재사용하되, release gate wrapper가 인자를 고정하고 결과 JSON과 Prometheus query 결과를 함께 판정한다.
