...
class RedisMessageAcceptance(
    ...
    private val workerProperties: ChatWorkerProperties = ChatWorkerProperties(),
) {
    ...
    fun append(envelope: MessageStreamEnvelope): MessageStreamEnvelope {
        ...
        if (payload == "FULL") throw MessageAdmissionRejectedException("Message stream is full; retry after workers catch up")
    ...
    fun markDeadLettered(envelope: MessageStreamEnvelope) = markPersisted(envelope)
    ...
    private companion object {
        ...
        val ACCEPT = DefaultRedisScript<String>().apply {
            setLocation(ClassPathResource("redis/accept-message.lua"))
            resultType = String::class.java
        }

        // Pending messages never expire. DB commits and durable writer DLQ entries start retention;
